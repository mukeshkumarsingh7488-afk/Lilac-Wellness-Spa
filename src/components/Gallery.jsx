    ...
    <>
      <section id="gallery" className="zn-section zn-gallery">
        <div className="zn-section-head">
          <p>Gallery</p>
          <h2>Inside Lilac Wellness Spa</h2>
          <span />
          <p className="zn-gallery-subtitle">Step inside Lilac Wellness Spa and experience luxury redefined.</p>
        </div>

        <div className="zn-gallery-grid">
          {galleryImages.map((image, index) => (
            <div className="zn-gallery-item" key={image}>
              <img src={image} alt={`Lilac Wellness Spa Gallery ${index + 1}`} />
            </div>
          ))}
        </div>
      </section>
